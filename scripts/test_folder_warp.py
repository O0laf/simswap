...
from warping.face_landmark_detection import generate_face_correspondences
from warping.delaunay_triangulation import make_delaunay
from warping.face_morph import generate_morph_image
...
class FS():
    ...
    def load_FS_model(self):
        ...
        G.load_state_dict(torch.load('/home/compu/samplecode_faceshifter/training_result/id7/ckpt/G_180000.pt', map_location=self.device))
    ...
    def get_mask(self, size):
        ...
        # cv2.imwrite("ms.jpg", mask_grad*255)


        mask_grad2 = np.zeros([size, size], dtype=float)
        mask_grad2[size//16:15*size//16, size//16:15*size//16] = 1
        mask_grad2 = cv2.blur(mask_grad2.astype(np.float64)*255, (25,25)).astype(np.uint8)/255
        mask_grad2 = cv2.blur(mask_grad2.astype(np.float64)*255, (25,25)).astype(np.uint8)/255
        mask_grad2 = cv2.blur(mask_grad2.astype(np.float64)*255, (25,25)).astype(np.uint8)/255
        self.mask_grad2 = mask_grad2
    ...
    def get_warped_face(self, img, ref_img):
        try:
            _, _, lms = self.FD_model.detect(img, landmarks=True)
        ...
        [size, img1, img2, points1, points2, list3] = generate_face_correspondences(face1024.astype(np.uint8), (ref_img*255).astype(np.uint8))

        tri = make_delaunay(size[1], size[0], list3, img1, img2)

        warped_face1024 = generate_morph_image(img1, img2, points1, points2, tri).astype(np.uint8)[:,:, ::-1]
        
        warped_face256 = self.test_transform(Image.fromarray(warped_face1024))
        warped_face256 = warped_face256.unsqueeze(0).cuda()

        return warped_face1024/255, warped_face256, trans_inv

    def combine(self, Yt, trans_inv, Xt_raw):
        mask = cv2.warpAffine(self.mask_grad2, trans_inv, (np.size(Xt_raw, 1), np.size(Xt_raw, 0)), borderValue=(0, 0, 0))
...
if __name__ == "__main__":
    Xs_path = "samples/ref/00024.png"
    Xt_dir = "/home/compu/samplecode_faceshifter/samples/inputs"
    save_dir = "samples/outputs_20_warp_24"
    ...
    Xs1024, Xs256, _ = models.get_face(Xs_raw)

    Xt_paths = sorted(glob.glob(f"{Xt_dir}/*.*"))
    for Xt_path in Xt_paths:
        Xt_fname = os.path.split(Xt_path)[1][:-4]
        ...
        # get warped Xt
        Xt1024_warped, Xt256_warped, trans_inv = models.get_warped_face(Xt_raw, Xs1024)

        # get warped Yt
        Yt_warped = models.do_swap(Xs256, Xt256_warped)
        Yt_warped = models.do_sr(Yt_warped)
        Yt_warped = models.do_eye(Xt1024_warped, Yt_warped)
        Ytt_warped = models.combine(Yt_warped, trans_inv, Xt_raw)

        # get Yt
        save_path = f"{save_dir}/{Xt_fname}.jpg"
        
        cv2.imwrite(save_path, np.concatenate((Xs1024, Xt1024, Yt, Xt1024_warped, Yt_warped), axis=1)[:, :, ::-1]*255)
